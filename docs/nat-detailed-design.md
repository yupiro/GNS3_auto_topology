...
> **未実施**: 本書時点ではGNS3実機での試験は行っていない。以下はR1へのconfig投入後に実施すべき試験項目の一覧（[基本設計書 1.1](./nat-basic-design.md#11-目的)の検証観点に対応）。実施後、本表の「実施結果」「判定」列を更新すること。
...
| 4 | EXT-PC → `ping 203.0.113.100`（SVR1の静的NATアドレス） | 成功 | 未実施 | ⬜ 未実施 |
| 5 | R1 `show ip nat translations`（静的エントリ確認） | `192.168.1.100 <-> 203.0.113.100` が `---`（常時）表記で存在 | 未実施 | ⬜ 未実施 |
| 6 | EXT-PC → `ping 192.168.1.10`（PC1の内部アドレスに直接） | 失敗（対応する変換エントリなし） | 未実施 | ⬜ 未実施 |
| 7 | R1 `show ip nat statistics` | Total active translations / Hits / Misses が試験内容と整合 | 未実施 | ⬜ 未実施 |
